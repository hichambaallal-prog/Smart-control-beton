...
def show(supabase):
    ...
    if isinstance(user_raw, dict):
        user_raw = (
            user_raw.get("email") or user_raw.get("name") or "Agent LPEE"
        )
    ...
    if editing_item:
        ...
        default_date = (
            datetime.strptime(editing_item["date_essai"], "%Y-%m-%d").date()
            if isinstance(editing_item.get("date_essai"), str)
            else date.today()
        )
    ...
    # ---------------------------------------------------------
    # SYNCHRONISATION FORCÉE DU CHAMP TECHNICIEN DANS LE STATE
    # ---------------------------------------------------------
    # Si nous sommes hors édition ou si la session a changé, on applique direct au Widget
    if not editing_item or "plaque_tech" not in st.session_state:
        st.session_state["plaque_tech"] = default_tech
    ...
    st.subheader(
        "📝 "
        + ("Modifier l'essai" if editing_item else "Saisie d'un nouvel essai")
    )
    ...
    with col1:
        date_essai = st.date_input(
            "Date de l'essai", value=default_date, key="plaque_date"
        )
        client = st.text_input(
            "Client / Organisme", value=default_client, key="plaque_client"
        )
        projet = st.text_input(
            "Chantier / Projet", value=default_projet, key="plaque_projet"
        )

    with col2:
        emplacement = st.text_input(
            "Emplacement / Zone", value=default_empl, key="plaque_empl"
        )
        pk_profil = st.text_input(
            "PK / Profil", value=default_pk, key="plaque_pk"
        )
        couche_options = [
            "Arase",
            "Assise",
        ...
        couche_idx = (
            couche_options.index(default_couche)
            if default_couche in couche_options
            else 0
        )
        couche = st.selectbox(
            "Couche testée",
        ...
        nature_materiau = st.text_input(
            "Nature du matériau", value=default_mat, key="plaque_mat"
        )

    with col3:
        st.markdown("##### 📏 Données de Chargement (Enfoncements)")
        z1 = st.number_input(
            "Z1 - 1er chargement (mm)",
            min_value=0.01,
            max_value=10.0,
            value=default_z1,
            step=0.01,
        ...
        technicien = st.text_input(
            "Technicien LPEE", key="plaque_tech"
        )  # La valeur provient directement de st.session_state["plaque_tech"]
    ...
    res_col3.metric(
        "Coefficient K (EV2/EV1)",
        f"{k_ratio:.2f}",
        delta=k_delta,
        delta_color="normal" if k_ratio <= 2.0 else "inverse",
    )

    observations = st.text_area(
        "Observations / Remarques", value=default_obs, key="plaque_obs"
    )
    ...
    with btn_col1:
        button_label = (
            ...
            if editing_item
            else "💾 Enregistrer l'essai"
        )
        if st.button(
            button_label,
            key="btn_enregistrer_plaque",
            type="primary",
            ...
            try:
                ...
                sample_query = (
                    supabase.table("essais_plaque").select("*").limit(1).execute()
                )
                if sample_query.data and len(sample_query.data) > 0:
                    valid_columns = set(sample_query.data[0].keys())
                    safe_payload = {
                        k: v for k, v in payload.items() if k in valid_columns
                    }
                else:
                    safe_payload = payload

                if editing_item:
                    supabase.table("essais_plaque").update(safe_payload).eq(
                        "id", editing_item["id"]
                    ).execute()
                    st.success(
                        f"✅ Essai #{editing_item['id']} mis à jour avec succès !"
                    )
                ...
                # Nettoyage de la clé pour qu'elle se réinitialise à la prochaine soumission
                if "plaque_tech" in st.session_state:
                    del st.session_state["plaque_tech"]
    ...
    try:
        ...
        if res.data and len(res.data) > 0:
            ...
            for row in res.data:
                k_val = (
                    row.get("k_ratio")
                    if row.get("k_ratio") is not None
                    else row.get("k")
                )
                pk_val = (
                    row.get("pk_profil")
                    if row.get("pk_profil") is not None
                    else row.get("pkl")
                )
            ...
            if is_admin:
                st.markdown(
                    "### ⚙️ Actions Administrateur (Modifier / Supprimer)"
                )
                ...
                with act_col2:
                    if st.button(
                        "🗑️ Supprimer cet essai",
                        type="primary",
                        use_container_width=True,
                    ):
                        try:
                            supabase.table("essais_plaque").delete().eq(
                                "id", selected_id
                            ).execute()
                            st.success(
                                f"🗑️ Essai #{selected_id} supprimé avec succès."
                            )
                            st.rerun()
                        except Exception as e:
