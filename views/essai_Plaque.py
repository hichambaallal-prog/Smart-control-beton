...
def show(supabase):
    ...
    # ---------------------------------------------------------
    # Récupération sécurisée du nom d'utilisateur connecté
    user_raw = (
    ...
    # Si la valeur récupérée est un dictionnaire (ex: objet Supabase / Auth)
    if isinstance(user_raw, dict):
        user_raw = user_raw.get("email") or user_raw.get("name") or "Agent LPEE"

    # Conversion en chaîne de caractères sécurisée avant d'appliquer .upper()
    ...
    if editing_item:
        ...
        default_z1 = 0.53
        default_z2 = 0.52
        default_tech = current_user  # Remplissage automatique avec l'utilisateur actif
