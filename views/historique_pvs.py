  ...
  try:
    ...
    if not df_valides.empty:
      ...
      if not pvs_filtrés:
        ...
        ouv_h = (
            info_b_h.get("ouvrage") if info_b_h else None
        ...
        infos_header_h = {
            ...
            "date_coulee": (
                info_b_h.get("date_coulee") if info_b_h else None
            )
            ...
            "centrale": (
                (info_b_h.get("centrale") or info_b_h.get("centrale_beton"))
                if info_b_h
        ...
        # --- CONTRÔLE DE VALIDATION ADMINISTRATEUR ---
        est_valide = (
            sample_h.get("validation_admin", False)
            or (info_b_h or {}).get("validation_admin", False)
            or is_admin
        )
        ...
        if est_valide:
          ...
          st.error(
              "⛔ **Téléchargement impossible** : Ce PV n'a pas encore été"
    ...
    # Mapping dynamique des fiches parents
    unique_parents = {
        b_id: obtenir_infos_betonnage_parent(supabase, b_id)
        for b_id in df_all["betonnage_id"].unique()
        if b_id
    }
    ...
    df_all["statut_validation"] = df_all.apply(
        lambda r: (
            "✅ Validé"
            if r.get("validation_admin")
            or (unique_parents.get(r.get("betonnage_id")) or {}).get(
                "validation_admin"
            )
            else "⏳ En attente"
        ),
