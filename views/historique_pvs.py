...
def verifier_doublon_num_reception(supabase, num_reception, current_beton_id=None):
    ...
    if not num_clean or num_clean.upper() in ["-", "NONE", "NAN", "N/A"]:
        return False
    try:
        res = supabase.table("suivi_betonnage").select("id, num_reception").eq("num_reception", num_clean).execute()
...
def extraire_num_bl(*sources):
    ...
    clefs = {"num_bl", "bl", "num_bon_livraison", "n_bl", "bon_livraison", "num_bl_p", "n_bon", "bon_de_livraison", "code_bl"}
...
def generer_pv_excel(export_data, infos_header):
    ...
    def set_cell(coord_or_cell, val=None, font=f_reg, align=a_center, fill=None, border=b_cell):
    ...
    # En-tête
    ws.merge_cells("A1:D1"); set_cell("A1", "LPEE / CTR CSB", f_bold_w, fill=fill_dark)
    ws.merge_cells("A2:D3"); set_cell("A2", "Laboratoire de Contrôle Externe", f_bold_w, fill=fill_dark)
    for r in range(1, 4):
        for c in range(1, 9):
    ...
    ws.merge_cells("F1:G1"); set_cell("F1", clean_na(infos_header.get("re_num"), "25/260/LGV/ B/"))
    set_cell("H1", "BETON", f_bold)

    set_cell("E2", "DOSSIER :", f_bold)
    ws.merge_cells("F2:H2"); set_cell("F2", clean_na(infos_header.get("dossier"), "2025-260-05985-2025-0247"))

    set_cell("E3", "CLIENT :", f_bold)
    ws.merge_cells("F3:H3"); set_cell("F3", clean_na(infos_header.get("client"), "TGCC"), f_bold)

    # Titre & Normes
    ws.merge_cells("A4:H4"); set_cell("A4", "ESSAIS MECANIQUES SUR BETON HYDRAULIQUE", f_title_w, fill=fill_dark)
    for c in range(1, 9): ws.cell(row=4, column=c).border = b_cell

    ws.merge_cells("A5:D5"); set_cell("A5", "[X] COMPRESSION NF EN 12390-3 (2019)", f_bold)
    ws.merge_cells("E5:H5"); set_cell("E5", "[ ] TRACTION PAR FENDAGE NF EN 12390-6 (2019)", f_bold)
    for c in range(1, 9): ws.cell(row=5, column=c).border = b_cell

    ws.merge_cells("A6:F6"); set_cell("A6", "Presse : Marque: Controls", f_bold, a_right)
    ws.merge_cells("G6:H6"); set_cell("G6", "Classe : A", f_bold)
    for c in range(1, 9): ws.cell(row=6, column=c).border = b_cell

    # Fiche technique
    set_cell("A7", "Date de\nprélèvement", f_bold, fill=fill_label)
    set_cell("B7", str(clean_na(infos_header.get("date_coulee"), "-")), f_bold)
    ws.merge_cells("C7:D7"); set_cell("C7", "Lieu de\nprélèvement", f_bold, fill=fill_label)
    ws.merge_cells("E7:H7"); set_cell("E7", clean_na(infos_header.get("lieu_prelevement", infos_header.get("ouvrage")), "-"))

    set_cell("A8", "Chantier", f_bold, fill=fill_label)
    ws.merge_cells("B8:D8"); set_cell("B8", clean_na(infos_header.get("chantier"), "Augmentation de la capacité ferroviaire..."), f_small)
    ws.merge_cells("E8:F8"); set_cell("E8", "Type de béton", f_bold, fill=fill_label)
    ws.merge_cells("G8:H8"); set_cell("G8", str(clean_na(infos_header.get("classe_beton"), "C35/45")).upper(), f_bold)

    ws.merge_cells("A9:B9"); set_cell("A9", clean_na(infos_header.get("centrale"), "Centrale à Béton"), f_bold, fill=fill_label)
    set_cell("C9", "- Dimensions", align=a_left)
    ws.merge_cells("D9:H9"); set_cell("D9", clean_na(infos_header.get("forme"), "Cylindrique 150x300"), f_bold)

    ws.merge_cells("A10:B10"); set_cell("A10", "Affaissement au cône d'abrams NF EN 12350-2", f_small, fill=fill_label)
    set_cell("C10", str(clean_na(infos_header.get("affaissement"), "-")), f_bold)
    set_cell("D10", "- Mode confection", align=a_left)
    ws.merge_cells("E10:H10"); set_cell("E10", "Par vibration NF EN 12390-2 (2019)", f_bold)

    ws.merge_cells("A11:B11"); set_cell("A11", "Température °C", fill=fill_label)
    set_cell("C11", str(clean_na(infos_header.get("temperature"), "-")), f_bold)
    set_cell("D11", "- Mode conservation", align=a_left)
    ws.merge_cells("E11:H11"); set_cell("E11", "au laboratoire par immersion dans l'eau NF EN 12390-2 (2019) à 20°C ± 2°C", f_bold)

    tech = clean_na(infos_header.get("technicien_prelevement") or infos_header.get("preleve_par") or infos_header.get("technicien"), "Technicien LPEE")
    ws.merge_cells("A12:C12"); set_cell("A12", f"prélèvement effectué par {tech}", f_small, fill=fill_label)
    ws.merge_cells("D12:E12"); set_cell("D12", "N° de bon de livraison", fill=fill_label)
    ws.merge_cells("F12:H12"); set_cell("F12", default_bl, f_bold)

    for r in range(7, 13):
        for c in range(1, 9): ws.cell(row=r, column=c).border = b_cell

    # Tableau des résultats
    headers = [("A13:A14", "Réf,", f_bold), ("B13:C13", "Date", f_bold), ("B14", "Fabri", f_reg),
               ("C14", "Essai", f_reg), ("D13:D14", "Age (jours)", f_bold), ("E13:E14", "Charge rupture(KN)", f_bold),
               ("F13:H13", "Résistance (MPa)", f_bold), ("F14", "Compression", f_reg), ("G14", "Traction", f_reg), ("H14", "Moyenne", f_reg)]
    ...
    for idx, item in enumerate(export_data):
        ...
        age_val = int(str(item.get("age", 7)).replace("j", "").replace("jours", "").strip()) if str(item.get("age", 7)).isdigit() else item.get("age", 7)
        ...
        if is_en_cours:
            ...
            set_cell(f"E{r}", f_kn); ws[f"E{r}"].number_format = "0.0"
            set_cell(f"F{r}", float(item.get("fc_mpa", 0.0))); ws[f"F{r}"].number_format = "0.0"

        set_cell(f"G{r}", "-")
        for c in range(1, 9): ws.cell(row=r, column=c).border = b_cell

        cle = f"{item.get('age')}_{dt_essai}"
        groupes_lots.setdefault(cle, {"lignes": [], "en_cours": is_en_cours, "age": age_val})["lignes"].append(r)
    ...
    for data in groupes_lots.values():
        ...
        if data["en_cours"]:
            ...
            formula = f"=ROUND(F{start_r}, 1)" if start_r == end_r else f"=ROUND(AVERAGE(F{start_r}:F{end_r}), 1)"
    ...
    seuil = next((s for k, s in [("C25/30", 25.0), ("C30/37", 30.0), ("C35/45", 35.0), ("C40/50", 40.0)] if k in str(clean_na(infos_header.get("classe_beton"), "C35/45")).upper()), 35.0)

    comment_formula = "PERFORMANCES MECANIQUES A 28 JOURS SERONT DONNES ULTERIEUREMENT." if not a_des_28j or not cel_moyenne_28j else (
        ...
        f'IF({cel_moyenne_28j}>={seuil}, "{obs_defaut}", "PERFORMANCES MECANIQUES NON CONFORMES"))'
    )
    set_cell(f"B{next_r}", comment_formula, f_bold, a_left)
    for c in range(1, 9): ws.cell(row=next_r, column=c).border = b_cell
...
def obtenir_historique_betonnage(supabase, betonnage_id):
    ...
    try:
        res = supabase.table("suivi_controle_beton").select("*").eq("betonnage_id", betonnage_id).order("id").execute()
        return res.data or []
    except Exception:
        return []
...
def determiner_ref_controle(supabase, betonnage_id, info_betonnage, sample_ep):
    ...
    if num_rec and str(num_rec).strip().upper() not in ["", "-", "NONE", "NAN", "N/A"]:
        ...
        ref = ((info_betonnage or {}).get("ref_controle") or (sample_ep or {}).get("ref_controle") or f"REF-{betonnage_id}-{(info_betonnage or {}).get('ouvrage', 'N/A')}").strip()
...
def show(supabase):
    ...
    role = str(st.session_state.get("user_role") or st.session_state.get("role") or user_info.get("role", "")).lower()
    can_edit = st.session_state.get("can_edit", False) or bool(user_info.get("can_edit", False))

    if role not in ["laboratoire", "labo", "admin", "responsable_labo", "qualite"] and not st.session_state.get("is_admin", False) and not can_edit:
    ...
    try:
        ...
        if not df_valides.empty:
            ...
            recherche_pv = c_r1.text_input("🔍 Rechercher (réf, ouvrage, classe...)", placeholder="Ex: gare casa sud, B/394...", key="search_input_pv")
            recherche_date_pv = c_r2.text_input("📅 Rechercher par Date d'écrasement", placeholder="Ex: 2026-08-08", key="search_date_pv")
            ...
            for _, row in df_valides.iterrows():
                ...
                classe = row.get("classe_beton") or (info_b.get("classe_beton") or info_b.get("classe") if info_b else "-") or "-"

                cle = f"Référence : {ref_ctrl} | Classe : {classe} | Ouvrage : {row.get('ouvrage', '-')} | Échéance : {row.get('echeance', '28 jours')} (Date : {row.get('date_ecrasement', '-')}) | Lot ID #{b_id}"
                groupes_valides.setdefault(cle, []).append(row.to_dict())

            pvs_filtrés = [k for k in groupes_valides.keys() if (not recherche_pv or recherche_pv.lower() in k.lower()) and (not recherche_date_pv or recherche_date_pv in k)]

            if not pvs_filtrés:
                st.warning("Aucun PV validé ne correspond à votre recherche.")
            else:
                choix_pv = st.selectbox("Sélectionnez le PV validé à re-télécharger :", pvs_filtrés, key="select_pv_hist")
                lot_hist = groupes_valides[choix_pv]
                sample_h = lot_hist[0]
                b_id_h = sample_h.get("betonnage_id")

                info_b_h = obtenir_infos_betonnage_parent(supabase, b_id_h)
                ...
                num_bl_h = extraire_num_bl(sample_h, info_b_h or {}, choix_pv)
                ouv_h = (info_b_h.get("ouvrage") if info_b_h else None) or sample_h.get("ouvrage")

                infos_header_h = {
                    "re_num": "25/260/LGV/ B/", "dossier": "2025-260-05985-2025-0247", "client": "TGCC",
                    "num_bl": num_bl_h, "ouvrage": ouv_h, "lieu_prelevement": ouv_h,
                    "classe_beton": sample_h.get("classe_beton", "C35/45"),
                    "date_coulee": (info_b_h.get("date_coulee") if info_b_h else None) or sample_h.get("date_coulee"),
                    "affaissement": (info_b_h.get("affaissement") or info_b_h.get("slump")) if info_b_h else None,
                    "temperature": (info_b_h.get("temperature") or info_b_h.get("temp_beton")) if info_b_h else None,
                    "forme": sample_h.get("forme", "Cylindrique 150x300"),
                    "centrale": (info_b_h.get("centrale") or info_b_h.get("centrale_beton")) if info_b_h else sample_h.get("centrale"),
                    "observations": sample_h.get("observations", "PERFORMANCES MECANIQUES A 28 JOURS SONT CONFORMES"),
                    "technicien_prelevement": (info_b_h.get("technicien_prelevement") or info_b_h.get("preleve_par") or info_b_h.get("technicien")) if info_b_h else sample_h.get("technicien"),
                }

                st.download_button(
                    label="📄 Télécharger le PV (Excel Format LPEE)",
                    data=generer_pv_excel(export_data_h, infos_header_h),
                    file_name=f"PV_Ecrasement_RE-EXPORT_{num_bl_h if num_bl_h != '-' else 'BL'}.xlsx",
        ...
        # Map parent information dynamically via vectorization/dict lookup
        unique_parents = {b_id: obtenir_infos_betonnage_parent(supabase, b_id) for b_id in df_all["betonnage_id"].unique() if b_id}
        
        df_all["affaissement_mm"] = df_all["betonnage_id"].map(lambda b: (unique_parents.get(b) or {}).get("affaissement") or (unique_parents.get(b) or {}).get("slump") or "-")
        df_all["temp_beton_C"] = df_all["betonnage_id"].map(lambda b: (unique_parents.get(b) or {}).get("temperature") or (unique_parents.get(b) or {}).get("temp_beton") or "-")

        cols_ordre = ["id", "betonnage_id", "repere_eprouvette", "num_bl", "ouvrage", "classe_beton", "date_coulee", "affaissement_mm", "temp_beton_C", "echeance", "date_ecrasement", "fc_mpa", "technicien"]
        exclus = {"forme", "section", "force_kn", "observations", "masse", "ref_controle", "reference_controle", "refernce_controle", "num_reception"}

        cols_finales = [c for c in cols_ordre + [c for c in df_all.columns if c not in cols_ordre] if c not in exclus]
        df_final = df_all[cols_finales]

        c_s1, c_s2 = st.columns(2)
        search_ref = c_s1.text_input("🔍 Recherche par Réf. Contrôle", placeholder="Ex: REF-123-GARE CASA SUD")
        search_date = c_s2.text_input("📅 Recherche par Date de coulée", placeholder="Ex: 2026-08-24")

        if search_ref:
            ref_col = next((c for c in ["ref_controle", "reference_controle", "refernce_controle"] if c in df_all.columns), None)
            if ref_col:
                df_final = df_final[df_all[ref_col].astype(str).str.contains(search_ref, case=False, na=False)]
        if search_date:
